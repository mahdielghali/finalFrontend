...
export class TransactionListComponent {
  ...
  this.titre="generate PDF",translateService.setDefaultLang('en-US');}
    ...
    private getTransactions(){
     ...
     getAmount(){
      ...
      this.transactionService.getTotalAmount().subscribe((amount: number) => {
       this.total=amount;
      });
    ...
    deleteTransaction(selectedTransactionId: any){
      this.isVisible=false;
      this.transactionService.deleteTransaction(selectedTransactionId).subscribe( data => {
        console.log(data);
        this.getTransactions();
    ...
    detailsTransaction(id: number){
      this.router.navigate(['detailsTransaction', id]);
...
navigateToCreateTransactionInterface(){
 ...
 makePDF() {
  ...
  html2canvas(table, {
    ...
    pdf.setFontSize(12);
    pdf.text('Nombre des transactions :'+this.size,405,790);
    ...
    pdf.setDrawColor(0, 0, 0);
    pdf.setFillColor(220, 220, 220);
    pdf.rect(400, 737, 160, 20, 'FD');
    pdf.setTextColor(0, 0, 0);
    pdf.setFontSize(12);
    pdf.text('Montant total:'+this.total, 405, 750);
